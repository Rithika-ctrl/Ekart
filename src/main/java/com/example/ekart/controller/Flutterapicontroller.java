...
public class FlutterApiController {
    ...
    public ResponseEntity<Map<String, Object>> getProfile(@RequestHeader("X-Customer-Id") int customerId) {
        ...
        profile.put("addresses", customer.getAddresses().stream().map(a -> {
            Map<String, Object> am = new HashMap<>(); am.put("id", a.getId()); am.put("details", a.getDetails()); return am;
        }).collect(Collectors.toList()));
    ...
    public ResponseEntity<Map<String, Object>> addAddress(
        ...
        if (details == null || details.isBlank()) { res.put("success", false); res.put("message", "Address cannot be empty"); return ResponseEntity.badRequest().body(res); }
        Address address = new Address();
        address.setDetails(details); address.setCustomer(customer);
