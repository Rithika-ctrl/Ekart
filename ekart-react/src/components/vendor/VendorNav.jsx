...
export default function VendorNav({ alertCount = 0 }) {
  ...
  return (
    <nav className={`vnd-nav${scrolled ? ' scrolled' : ''}`}>
      <Link to="/vendor" className="vnd-nav-brand">
      ...
      <div className="vnd-nav-right">
        ...
        <Link to="/vendor/report"  className={isActive('/vendor/report')}>
          <i className="fas fa-chart-line" aria-hidden="true" /> Sales
        </Link>
        <Link to="/vendor/storefront"   className={isActive('/vendor/storefront')}>
          <i className="fas fa-store" aria-hidden="true" /> Store
        </Link>
        {alertCount > 0 && (
          <Link to="/vendor" className="vnd-nav-link" style={{ position: 'relative' }}>
